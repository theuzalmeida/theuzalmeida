<table>
  <tr>
    <td  width="400">
      <img src="Inferno Canto VIII - Gustave Doré.jpeg" width="400" alt="Inferno Canto VIII - Gustave Doré">
    </td>
    <td valign="top">
      <h1>Matheus A. Almeida</h1>
      <p>Desenvolvedor full-stack com formação técnica em desenvolvimento de sistemas e estudante de ciência de dados. Atuo no desenvolvimento de software, interfaces web e automações, com foco na resolução de problemas por meio de código. Tenho interesse em desenvolvimento web, análise de dados e aprimoramento contínuo. Sou falando nativo de português e possuo nível intermediário de inglês</p>
      <h2>Áreas de Interesse</h2>
      <ul>
        <li>Desenvolvimento de software</li>
        <li>Desenvolvimento web</li>
        <li>Ciência e análise de dados</li>
        <li>Automação de processos</li>
        <li>UI/UX</li>
      </ul>
      <h2>Contato</h2>
      <ul>
        <li>Instagram: <a href="https://www.instagram.com/mthzip/">Mthzip</a></li>
        <li>LinkedIn: <a href="https://www.linkedin.com/in/matheus-almeida-99b590373/">Matheus A. Almeida</a></li>
        <li>Website: <a href="https://matheusalmeida.vercel.app/">Matheusalmeida.vercel.app</a></li>
      </ul>
    </td>
  </tr>
</table>
